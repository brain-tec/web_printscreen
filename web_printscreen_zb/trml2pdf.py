...
class NumberedCanvas(canvas.Canvas):
    ...
    def pageCount(self):
        """
        Add the page count to the footer of the rml document
        """
        ...
        self.drawRightString(
            self._pagesize[0] - 30, 40,
            " %(current_page)i / %(number_of_pages)i" % {
    ...
    def __init__(self, nodes, localcontext):
        ...
        for node in nodes:
            for style in node.findall('blockTableStyle'):
                self.table_styles[
                    style.get('id')] = self._table_style_get(style)
            for style in node.findall('paraStyle'):
                sname = style.get('name')
                self.styles[sname] = self._para_style_update(style)

                self.styles_obj[sname] = reportlab.lib.styles.ParagraphStyle(
                    sname, self.default_style["Normal"], **self.styles[sname])
...
class _rml_doc(object):
    ...
    def __init__(
        self, node, localcontext=None, images=None, path='.', title=None
    ):
        if images is None:
            images = {}
    ...
    def render(self, out):
        ...
        if len(el):
            pt_obj = _rml_template(
                self.localcontext, out, el[0],
            ...
            pd_obj = _rml_canvas(
                self.canvas, self.localcontext, None,
                self, self.images, path=self.path, title=self.title)
            pd_obj.render(pd)
...
class _rml_canvas(object):
    ...
    def __init__(
        self, canvas, localcontext, doc_tmpl=None, doc=None,
    ...
    def _rect(self, node):
        if node.get('round'):
            self.canvas.roundRect(
                radius=utils.unit_get(node.get('round')),
    ...
    def _ellipse(self, node):
        ...
        self.canvas.ellipse(
            x1, y1, x2, y2, **utils.attr_get(node, [], {
                'fill': 'bool', 'stroke': 'bool'}))
...
class _rml_Illustration(platypus.flowables.Flowable):
    ...
    def draw(self):
        drw = _rml_draw(
            ...
            self.node,
            self.styles,
            images=self.self2.images,
            path=self.self2.path,
            title=self.self2.title)
...
class _rml_flowable(object):
    ...
    def _table(self, node):
        ...
        for tr in children:
            ...
            if tr.get('style'):
                ...
                styles.append(st)
            if tr.get('paraStyle'):
                para_style = self.styles.styles[tr.get('paraStyle')]
            data2 = []
            posx = 0
            for td in utils._child_get(tr, self, 'td'):
                ...
                if td.get('paraStyle'):
                    # TODO: merge styles
                    para_style = self.styles.styles[td.get('paraStyle')]
                posx += 1
                ...
                for n in utils._child_get(td, self):
                    if n.tag == etree.Comment:
                        n.text = ''
                        continue
                    fl = self._flowable(n, extra_style=para_style)
        ...
        table = platypus.LongTable(
            data=data, colWidths=colwidths,
    ...
    def _pto(self, node):
        ...
        return platypus.flowables.PTOContainer(
            sub_story,
            trailer=pto_trailer,
            header=pto_header)
    ...
    def _flowable(self, node, extra_style=None):
        ...
        if node.tag == 'para':
            ...
            for i in self._textual(node).split('\n'):
                result.append(platypus.Paragraph(
                    i, style, **(utils.attr_get(
                        node, [],
                        {'bulletText': 'str'}))))
            return result
            ...
            codes = {
                ...
                'ean13': lambda x: createBarcodeDrawing(
                    'EAN13', value=str(x), **args),
                'qrcode': lambda x: createBarcodeDrawing(
                    'QR', value=x, **args),
            }
            code = 'code128'
            if node.get('code'):
            ...
            return platypus.XPreformatted(
                self._textual(node), style, **(utils.attr_get(
                    node, [], {
                        'bulletText': 'str',
                        'dedent': 'int',
                        'frags': 'int'
            ...
            style = self.styles.para_style_get(node)
            return platypus.Preformatted(self._textual(
                node), style, **(utils.attr_get(node, [], {
            ...
            if not node.get('file'):
                if node.get('name'):
                    if node.get('name') in self.doc.images:
                        _logger.info("Image %s read ", node.get('name'))
                    ...
                    image_data = base64.decodestring(newtext)
                if not image_data:
                    _logger.info("No inline image data")
                    return False
                image = StringIO(image_data)
            else:
                _logger.info("Image get from file %s", node.get('file'))
                image = _open_image(node.get('file'), path=self.doc.path)
            return platypus.Image(image, mask=(
                250, 255, 250, 255, 250, 255), **(utils.attr_get(
                    node, ['width', 'height'])))
            ...
            # TODO: change the 1000 !
            return platypus.CondPageBreak(1000)
...
class TinyDocTemplate(platypus.BaseDocTemplate):
    ...
    def afterPage(self):
        if self.canv._doPageReset:
            ...
            #   count itself)
            # NOTE: _rml_template render() method add a PageReset flowable at
            # end
            # of each story, so we're sure to pass here at least once per
            # story.

            if not isinstance(self.canv, NumberedCanvas):
                self.handle_flowable(
                    [PageCount(story_count=self.canv._storyCount)])
...
class _rml_template(object):
    ...
    def __init__(
        ...
        page_size = (841.8897637795275, 595.275590551181)
        ...
        for pt in pts:
            ...
            if len(gr):
                ...
                self.page_templates.append(platypus.PageTemplate(
                    frames=frames, onPage=drw.render,
